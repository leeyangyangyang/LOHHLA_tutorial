...
for (p in c('seqinr', 'Biostrings', 'beeswarm', 'zoo', 'Rsamtools', 'plyr','dplyr',
...
get.partially.matching.reads <- function(workDir, regionDir, BAMDir, BAMfile){
  ...
  cmd <- paste('picard SamToFastq',
               ' -I ',regionDir, '/fished.sam -F ', regionDir, '/fished.1.fastq -F2 ',
               regionDir, '/fished.2.fastq --VALIDATION_STRINGENCY SILENT', sep = '')
  system(cmd)
...
if(mapping.step){
  ...
  for(BAMfile in BAMfiles){
    ...
    for (i in seq_along(hla_locs)) {
      samtoolsCMD <- paste('samtools view ', BAMDir, '/', BAMfile, ' ',
                           hla_locs[i], ' >> ', regionDir,"/",BAMid,".chr6region.sam", sep = '')
      system(samtoolsCMD)
      print(samtoolsCMD)
    }
    ...
    print(alt_loci_names)
    
    if (length(alt_loci_names) == 0) {
      print("no alternate loci")

    } else {
      ## Add reads mapped to alternate loci to fished file
      print('Add reads mapped to alternate loci to fished file')
      for (chrom_name in alt_loci_names) {
        samtoolsCMD <- paste0('samtools view ', BAMDir, '/', BAMfile,
                              ' ', chrom_name, ' >> ',regionDir,"/",BAMid,".chr6region.sam")
        system(samtoolsCMD)
        print(samtoolsCMD)
      }
    }
    ...
    samToFastQ <- paste("picard SamToFastq ","-I ",regionDir,"/",BAMid,".chr6region.sam"," -F ",regionDir,"/",BAMid,".chr6region.1.fastq"," -F2 ",regionDir,"/",BAMid,".chr6region.2.fastq"," --VALIDATION_STRINGENCY SILENT",sep="")
    ...
    # sort
    sortBAM <- paste("picard SortSam"," -I ",regionDir, '/', BAMid, '.chr6region.patient.reference.hlas.bam'," ","-O ",regionDir,"/",BAMid,".chr6region.patient.reference.hlas.csorted.bam", " --SORT_ORDER coordinate",sep="")
    ...
    for (allele in hlaAlleles)
    {
      ...
      extractCMD <- paste("picard FilterSamReads ", "-I ",  regionDir, '/', BAMid, '.type.', allele, '.bam' , " -FILTER includeReadList -READ_LIST_FILE ", regionDir, "/", BAMid, '.', allele, ".passed.reads.txt", ' -OUTPUT ', regionDir, '/', BAMid, '.type.', allele, '.filtered.bam', sep = '')
...
for (region in regions)
{
  ...
  BAMfiles  <- grep('filtered.bam$', grep('type',list.files(regionDir,recursive = T),value=TRUE),value=TRUE)
  
  print(paste(BAMDir, '/', region, '.bam', sep = ''))
  print(normalBAMfile)
...
for (region in regions)
{
  ...
  {  
    ...
    for (HLA_gene in c('hla_a','hla_b','hla_c'))
    {
      ...
      PatientOutPut    <-unique(PatientOutPut)
  ...
  if(plottingStep)
  {
    ...
    for (HLA_gene in unique(substr(hlaAlleles, 1,5)))
    {
      ...
      HLABAFsummaryLoc <- paste(workDir, '/', full.patient,'.', minCoverageFilter,'.',HLA_gene,".DNA.IntegerCPN_CI.xls",sep="")
